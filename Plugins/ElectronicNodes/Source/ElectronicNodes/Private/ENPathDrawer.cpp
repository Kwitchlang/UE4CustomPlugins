...
void FENPathDrawer::DrawManhattanWire(const FVector2D& Start, const FVector2D& StartDirection, const FVector2D& End, const FVector2D& EndDirection)
{
	...
	bool SameDirection = FVector2D::DistSquared(StartDirection, EndDirection) < KINDA_SMALL_NUMBER;
	bool StraightDirection = FMath::IsNearlyZero(FVector2D::CrossProduct(StartDirection, EndDirection), KINDA_SMALL_NUMBER);
	bool ForwardDirection = FVector2D::DotProduct(End - Start, StartDirection) > KINDA_SMALL_NUMBER;

	float DistanceOrtho = FVector2D::CrossProduct(End - Start, StartDirection);
	float DistanceStraight = FVector2D::DotProduct(End - Start, StartDirection);

	FVector2D new_Start = Start, new_StartDirection = StartDirection;
	FVector2D new_End = End, new_EndDirection = EndDirection;

	int32 DirectionAngle = FMath::Sign(DistanceOrtho);
	...
	if (SameDirection)
	{
		...
		if (FMath::IsNearlyEqual(FVector2D::DotProduct((End - Start).GetSafeNormal(), StartDirection), 1.0f, KINDA_SMALL_NUMBER))
		{
			DrawLine(Start, End);
			return;
		}
		else if (FMath::IsNearlyEqual(FVector2D::DotProduct((End - Start).GetSafeNormal(), StartDirection), -1.0f, KINDA_SMALL_NUMBER))
		{
			...
			DrawSimpleRadius(Start, StartDirection, -90, new_Start, new_StartDirection, false);
			DrawSimpleRadius(End, EndDirection, 90, new_End, new_EndDirection, true);
		}
		else if ((FMath::Abs(End.X - Start.X) < 2 * GetRadiusOffset()) && (FMath::Abs(End.Y - Start.Y)) < 4 * GetRadiusOffset())
		{
			float Multiplier = FVector2D::Distance(Start, End) / 32.0f;
		...
		else if (!ForwardDirection && (FMath::Abs(DistanceOrtho) < 4.0f * GetRadiusOffset()))
		{
			...
			DrawUTurn(Start, StartDirection, DirectionAngle, new_Start, new_StartDirection, false);
		}
		else if (FMath::Abs(End.Y - Start.Y) < 2.0f * GetRadiusOffset())
		{
			DrawCorrectionOrtho(End, EndDirection, DistanceOrtho, new_End, new_EndDirection, true);
		}
		else
		{
			...
			if (DistanceStraight < 2.0f * GetRadiusOffset())
			{
				...
				{
					DrawUTurn(End, EndDirection, DirectionAngle, new_End, new_EndDirection, true);
				}
			}
			else
			{
				float Direction = -FMath::Sign(DistanceOrtho);

				if (RightPriority)
				{
					DrawSimpleRadius(End, EndDirection, 90 * Direction, new_End, new_EndDirection, true);
				}
				else
				{
					DrawSimpleRadius(Start, StartDirection, 90 * Direction, new_Start, new_StartDirection, false);
				}
			}
		}
	}
	...
	else
	{
		if (FMath::Sign(DistanceStraight) > 0)
		{
			...
			DrawSimpleRadius(End, EndDirection, 90 * DirectionAngle, new_End, new_EndDirection, true);
		}
		else
		{
			DebugColor(FLinearColor(1.0f, 0, 1.0f));
			DrawSimpleRadius(Start, StartDirection, -90 * DirectionAngle, new_Start, new_StartDirection, false);
		}
	}

	DrawManhattanWire(new_Start, new_StartDirection, new_End, new_EndDirection);
}
...
void FENPathDrawer::DrawSubwayWire(const FVector2D& Start, const FVector2D& StartDirection, const FVector2D& End, const FVector2D& EndDirection)
{
	...
	bool StartDirection90 = FMath::IsNearlyEqual(FMath::Abs(StartDirection.X), 1.0f, KINDA_SMALL_NUMBER) || FMath::IsNearlyEqual(FMath::Abs(StartDirection.Y), 1.0f, KINDA_SMALL_NUMBER);
	bool EndDirection90 = FMath::IsNearlyEqual(FMath::Abs(EndDirection.X), 1.0f, KINDA_SMALL_NUMBER) || FMath::IsNearlyEqual(FMath::Abs(EndDirection.Y), 1.0f, KINDA_SMALL_NUMBER);
	...
	float DistanceOrtho = FVector2D::CrossProduct(End - Start, StartDirection);
	float DistanceStraight = FVector2D::DotProduct(End - Start, StartDirection);
	float DirectionOffset = (FMath::Abs(End.X - Start.X) - FMath::Abs(End.Y - Start.Y)) * (FMath::IsNearlyEqual(FMath::Abs(StartDirection.X), 1.0f, KINDA_SMALL_NUMBER) ? 1 : -1);
	...
	else if (SameDirection)
	{
		...
		else if (ForwardDirection && DirectionOffset > 2 * GetIntersectionOffset(45, false))
		{
			...
			else
			{
				if (RightPriority)
				{
					DrawSimpleRadius(End, EndDirection, -45 * DirectionAngle, new_End, new_EndDirection, true);
				}
				else
				{
					DrawSimpleRadius(Start, StartDirection, -45 * DirectionAngle, new_Start, new_StartDirection, false);
				}
			}
		}
		else
	...
	DrawSubwayWire(new_Start, new_StartDirection, new_End, new_EndDirection);
}
...
{
	int32 AngleDeg = FMath::RoundToInt(FMath::UnwindDegrees(FMath::RadiansToDegrees(FMath::Atan2(StartDirection.Y, StartDirection.X) - FMath::Atan2(EndDirection.Y, EndDirection.X))));

	float StartOffset = GetIntersectionOffset(AngleDeg, false);
	float EndOffset = GetIntersectionOffset(AngleDeg, true);
	...
	FVector2D StartStop = Intersection - StartDirection * StartOffset;
	FVector2D EndStop = Intersection + EndDirection * EndOffset;
...
{
	float DirectionOffset = FMath::Abs(End.X - Start.X) - FMath::Abs(End.Y - Start.Y);

	FVector2D new_Start = Start;
	FVector2D new_End = End;

	FVector2D new_StartClose, new_StartCloseDirection;
	FVector2D new_EndClose, new_EndCloseDirection;
	...
	if (FMath::IsNearlyEqual(FMath::Abs(StartDirection.X), 1.0f, KINDA_SMALL_NUMBER))
	{
		...
		{
			new_Start += FVector2D(1.0f, 0.0f) * DirectionOffset * StartDirection.X;
		}
		else
		{
			new_End += FVector2D(0.0f, 1.0f) * DirectionOffset * FMath::Sign(End.Y - Start.Y);
		}
	}
	else
	{
		...
		if (DirectionOffset > 0)
		{
			new_End -= FVector2D(1.0f, 0.0f) * DirectionOffset * EndDirection.X;
		}
		else
		{
			new_Start -= FVector2D(0.0f, 1.0f) * DirectionOffset * FMath::Sign(End.Y - Start.Y);
		}
	}

	DebugColor(FLinearColor(1.0f, 1.0f, 1.0f));
	DrawLine(Start, new_Start);
	DrawLine(new_End, End);

	DebugColor(FLinearColor(0.0f, 0.0f, 1.0f));
	DrawSimpleRadius(new_Start, StartDirection, 45 * Direction, new_StartClose, new_StartCloseDirection, false);
	DrawSimpleRadius(new_End, EndDirection, -45 * Direction, new_EndClose, new_EndCloseDirection, true);

	DebugColor(FLinearColor(1.0f, 1.0f, 1.0f));
	DrawLine(new_StartClose, new_EndClose);
}
...
void FENPathDrawer::DrawUTurn(const FVector2D& Start, const FVector2D& StartDirection, float Direction, FVector2D& out_End, FVector2D& out_EndDirection, bool Backward)
{
	...
	float BackwardDirection = Backward ? -1.0f : 1.0f;

	FVector2D MidDirection = StartDirection.GetRotated(FMath::Sign(Direction) * 90 * BackwardDirection);
	FVector2D Mid = Start + (StartDirection + MidDirection) * GetRadiusOffset() * BackwardDirection;
...
{
	out_EndDirection = StartDirection;
	FVector2D StartDirectionOrtho = StartDirection.GetRotated(90);
...
float FENPathDrawer::GetRadiusOffset(const int32& AngleDeg, bool Perpandicular)
{
...
float FENPathDrawer::GetIntersectionOffset(const int32& AngleDeg, bool Diagonal)
{
	...
	case 45:
		if (Diagonal)
		{
			IntersectionOffset *= (1.0f - FMath::Sqrt(2.0f) / 2.0f) * FMath::Sqrt(2);
		}
		else
		{
...
void FENPathDrawer::DrawOffset(FVector2D& Start, FVector2D& StartDirection, const float& Offset, bool Backward)
{
	...
	if (Backward)
	{
		new_Start -= StartDirection * Offset;
		DrawLine(new_Start, Start);
	}
	else
	{
		new_Start += StartDirection * Offset;
