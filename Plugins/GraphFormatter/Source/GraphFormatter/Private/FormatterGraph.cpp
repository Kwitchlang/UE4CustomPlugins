...
void FFormatterGraph::CreatePinsForGroup(FFormatterNode* GroupNode, const TSet<UEdGraphNode*>& OuterSelected, TArray<FFormatterPin*>& InPins, TArray<FFormatterPin*>& OutPins) const
{
	const TSet<UEdGraphNode*> InnerSelected = GetChildren(GroupNode->OriginalNode, OuterSelected);
	TArray<FFormatterPin*> Pins;
	for (auto Node : InnerSelected)
	{
		for (auto Pin : Node->Pins)
		{
			for (auto LinkedToPin : Pin->LinkedTo)
			{
				...
				if (InnerSelected.Contains(LinkedToNode) || !OuterSelected.Contains(LinkedToNode))
				{
					continue;
				}
				auto NewPin = new FFormatterPin();
